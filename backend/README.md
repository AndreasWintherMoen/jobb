...
Additionally, there's a main function called `api.py` which is a very simple Flask server that I run on a Linode server instead of a serverless Google Cloud function. The server listens to incoming subscribe/unsubscribe SMS, and updates the database accordingly.
