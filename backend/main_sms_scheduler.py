...
def schedule_sms_for_todays_events(data, context) -> str:
    custom_logger.info("********* SCHEDULING SMS FOR TODAY... *********")
    register_events, unattend_events, start_events = get_todays_events()
    custom_logger.info(f"Found {len(register_events)} register events, {len(unattend_events)} unattend events and {len(start_events)} start events to schedule SMS for")
    for date in register_events:
        time_to_send = get_delay_until_five_minutes_before_event(date)
        events = register_events[date]
        event_ids = [event['id'] for event in events]
        custom_logger.info(f"Sending sms for register events {[event['title'] for event in events]} at {date} in {time_to_send} seconds")
    ...
    for date in unattend_events:
        ...
        event_ids = [event['id'] for event in events]
        custom_logger.info(f"Sending sms for unattend events {[event['title'] for event in events]} at {date} in {time_to_send} seconds")
        schedule_external_sms_sender(event_ids, MessageType.UNATTEND, time_to_send)
    for date in start_events:
        time_to_send = get_delay_until_one_hour_before_event(date)
        events = start_events[date]
        event_ids = [event['id'] for event in events]
        custom_logger.info(f"Sending sms for start events {[event['title'] for event in events]} at {date} in {time_to_send} seconds")
